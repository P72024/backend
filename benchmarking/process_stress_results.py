...
def plot_scatter(df, x_col, y_col, xlabel, ylabel, title):
    ...
    plt.plot(df[x_col], regression_line, label='Regression Line (y={a:.2f}x^2 + {b:.2f}x + {c:.2f}')
    plt.text(10, 40.5, y, fontsize=12)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    plt.grid(True)
...
def run_process_stress_results(results):
    ...
    df_specific_num_of_rooms = df[df["num_concurrent_rooms"] == 32]
    df_specific_num_of_workers = df[df["num_workers"] == 16]
