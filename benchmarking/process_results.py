...
results_avg = pd.read_csv('./results/results_avg.csv')
...
config_params = ['Chunk Size', 'VAD Threshold', 'model_type', 'beam_size', 'use_context', 'confidence_based', 'num_workers', 'confidence_limit', 'Word Error Rate (WER)', 'Word Information Loss (WIL)', 'Avg. chunk time']
# Sort data by Word Error Rate (WER) for ordered plotting
results_avg_sorted: pd.DataFrame = results.sort_values(by=['Word Information Loss (WIL)'])
print(f"Lowest WER config:\n{results_avg_sorted.head(1)[config_params].to_string()}")
